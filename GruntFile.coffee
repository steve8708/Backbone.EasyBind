...
module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    coffee:
      compile:
        files:
          'backbone.easybind.js': [ 'src/backbone.easybind.coffee' ]
    ...
    uglify:
      main:
        files:
          'backbone.easybind.min.js': [ 'backbone.easybind.js' ]

    karma:
      options:
        browsers: [ 'PhantomJS' ]
        frameworks: [ 'mocha', 'chai' ]
        files: [ 'backbone.easybind.js', 'test/**/*.coffee' ]
    ...
    replace:
      ...
      debugJS:
        src: [ 'backbone.easybind.js' ]
        overwrite: true
        replacements: [
          from: /^/
          to: -> "/* backbone.easybind.js v#{ packageVersion() }" +
            " (coffeescript output) */ \n\n"
        ]

      minJS:
        src: [ 'backbone.easybind.min.js' ]
        overwrite: true
        replacements: [
          from: /^/
          to: -> "/* backbone.easybind.min.js v#{ packageVersion() } */ \n"
        ]
    ...
    bump:
      options:
        ...
        commitFiles: [
          'package.json'
          'src/backbone.easybind.coffee'
          'backbone.easybind.js'
          'backbone.easybind.min.js'
        ]
        pushTo: 'origin'
